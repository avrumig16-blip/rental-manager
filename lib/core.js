...
function jwt(){const app=process.env.ENABLE_BANKING_APP_ID,key=(process.env.ENABLE_BANKING_PRIVATE_KEY||'').replace(/\\n/g,'\n');if(!app||!key)throw new Error('Missing ENABLE_BANKING_APP_ID or ENABLE_BANKING_PRIVATE_KEY');const now=Math.floor(Date.now()/1000),issuedAt=now-60,header=b64url(JSON.stringify({typ:'JWT',alg:'RS256',kid:app})),body=b64url(JSON.stringify({iss:'enablebanking.com',aud:'api.enablebanking.com',iat:issuedAt,exp:issuedAt+3600})),input=`${header}.${body}`,sig=crypto.sign('RSA-SHA256',Buffer.from(input),key).toString('base64url');return `${input}.${sig}`}
async function eb(path,options={}){
  ...
  try{
    ...
    r=await fetch(API+path,{...fetchOptions,signal:fetchOptions.signal||controller.signal,headers:{Authorization:`Bearer ${jwt()}`,Accept:'application/json',...(fetchOptions.body?{'Content-Type':'application/json'}:{}),...(fetchOptions.headers||{})}});
  }catch(e){
    const err=new Error(e?.name==='AbortError'?`Enable Banking request timed out after ${timeoutMs}ms`:`Enable Banking network error: ${e?.message||e}`);
    err.code=e?.name==='AbortError'?'EB_TIMEOUT':'EB_NETWORK';
    throw err;
  ...
  try{data=txt?JSON.parse(txt):{}}catch{
    const err=new Error(`Enable Banking ${r.status}: provider returned a non-JSON response`);
    err.code='EB_INVALID_RESPONSE';err.status=r.status;err.contentType=ct;err.providerBody=String(txt||'').slice(0,500);
    throw err;
  }
  if(!r.ok){
    const err=new Error(`Enable Banking ${r.status}: ${data?.detail||data?.message||data?.error||'Request failed'}`);
    err.code=r.status===401||r.status===403?'EB_RECONNECT_REQUIRED':'EB_PROVIDER_ERROR';err.status=r.status;err.provider=data;
    throw err;
  }
  return data;
}
...
async function userContext(req){const user=await requireUser(req),p=(await supa(`user_profiles?user_id=eq.${encodeURIComponent(user.id)}&select=active_workspace_id`))?.[0];if(!p?.active_workspace_id)throw new Error('No active workspace');const w=(await supa(`workspaces?id=eq.${p.active_workspace_id}&select=id,status,owner_user_id`))?.[0];if(!w||w.status!=='active')throw new Error('Workspace is suspended');const access=user?.user_metadata?.mr4u_access?.[String(w.id)]||{};const role=String(w.owner_user_id)===String(user.id)?'owner':String(access.role||'viewer').toLowerCase();return{user,workspaceId:w.id,role,scope:access.scope||'all',buildingIds:access.building_ids||[]}}
...
async function requireBankPermission(req){const ctx=await userContext(req);if(!['owner','admin'].includes(ctx.role)||ctx.scope==='buildings')throw new Error('Banking requires owner or administrator access to the whole account.');if(!(await workspaceBankAutoEnabled(ctx.workspaceId)))throw new Error('Bank connections are not enabled for this account. Request an upgrade from ManageRent4U support.');return ctx}
...
async function workspaceBankMatchSettings(workspaceId){const w=(await supa(`workspaces?id=eq.${encodeURIComponent(workspaceId)}&select=owner_user_id,status`))?.[0];if(!w||w.status!=='active')return {mode:'reference_amount',auto_sync:true};const u=await authUserById(w.owner_user_id),raw=u?.user_metadata?.mr4u_bank_match_settings||{};const mode=['review_only','reference_amount','strict'].includes(String(raw.mode||''))?String(raw.mode):'reference_amount';return {mode,auto_sync:raw.auto_sync!==false}}
...
module.exports={eb,supa,requireUser,userContext,ownedConnection,ownedAccount,workspaceBankAutoEnabled,workspaceBankMatchSettings,requireBankPermission,makeState,verifyState};
...
async function authUserById(id){const k=key();const r=await fetch(`${base()}/auth/v1/admin/users/${encodeURIComponent(id)}`,{headers:{apikey:k,Authorization:`Bearer ${k}`}});if(!r.ok)return null;return r.json()}
async function workspaceReplyTo(workspaceId){try{const w=(await supa(`workspaces?id=eq.${encodeURIComponent(workspaceId)}&select=owner_user_id,status`))?.[0];if(!w||w.status!=='active')return'';const u=await authUserById(w.owner_user_id);const v=String(u?.user_metadata?.mr4u_email_reply_to||'').trim();return /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v)?v:''}catch{return''}}
...
module.exports={supa,requireUser,appUrl,trackingUrl,replyAddress,workspaceReplyTo,event,verifySvix};
