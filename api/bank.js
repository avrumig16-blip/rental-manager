...
const {eb,requireUser}=require('../lib/core');module.exports=async(req,res)=>{try{await requireUser(req);const country=String(req.query.country||'BE').toUpperCase(),psu=String(req.query.psu_type||'business');const d=await eb(`/aspsps?country=${encodeURIComponent(country)}&psu_type=${encodeURIComponent(psu)}`);res.status(200).json(d)}catch(e){res.status(401).json({error:e.message})}}

return module.exports; })();

handlers["callback"] = (() => { const module = {exports:{}}; const exports = module.exports;
const {eb,supa,verifyState}=require('../lib/core');
...
const {eb,ownedConnection,makeState}=require('../lib/core');module.exports=async(req,res)=>{try{const id=String(req.query.connection_id||''),name=String(req.query.name||'').trim(),country=String(req.query.country||'BE').toUpperCase();if(!/^\d+$/.test(id)||!name)return res.status(400).json({error:'Invalid bank or connection'});const {workspaceId}=await ownedConnection(req,id),base=process.env.BANK_APP_URL||`https://${req.headers.host}`;const data=await eb('/auth',{method:'POST',body:JSON.stringify({access:{valid_until:new Date(Date.now()+89*86400000).toISOString(),balances:true,transactions:true},aspsp:{name,country},state:makeState(id,workspaceId),redirect_url:`${base}/api/bank/callback`,psu_type:'business',language:'nl'})});res.status(200).json({url:data.url})}catch(e){res.status(400).json({error:e.message})}}

return module.exports; })();

handlers["sync"] = (() => { const module = {exports:{}}; const exports = module.exports;
const {eb,supa,ownedConnection,workspaceBankAutoEnabled}=require('../lib/core');
...
module.exports=async(req,res)=>{try{if(req.method!=="POST")return res.status(405).json({error:"POST required"});const id=String(req.query.connection_id||"");if(!/^\d+$/.test(id))return res.status(400).json({error:"Invalid connection"});const {workspaceId,connection:c}=await ownedConnection(req,id);if(!(await workspaceBankAutoEnabled(workspaceId)))throw new Error("Bank checking is not enabled for this account. Contact the ManageRent4U administrator.");if(!c?.external_session_id)throw new Error("Bank connection is not authorized");const accts=await supa(`bank_accounts?workspace_id=eq.${workspaceId}&connection_id=eq.${id}&select=*`);let imported=0,autoMatched=0;for(const a of accts||[]){if(a.active===false||!a.external_account_id)continue;try{const b=await eb(`/accounts/${encodeURIComponent(a.external_account_id)}/balances`);const bal=(b.balances||[]).find(x=>["CLBD","CLAV","ITAV"].includes(x.balance_type))||(b.balances||[])[0];if(bal)await supa(`bank_accounts?id=eq.${a.id}`,{method:"PATCH",body:JSON.stringify({balance:Number(bal.balance_amount?.amount||0),currency:bal.balance_amount?.currency||a.currency||"EUR",balance_updated_at:new Date().toISOString()})})}catch(e){console.error("balance",e.message)}let continuation=null,loops=0;do{let path=`/accounts/${encodeURIComponent(a.external_account_id)}/transactions?date_from=${new Date(Date.now()-120*86400000).toISOString().slice(0,10)}`;if(continuation)path+=`&continuation_key=${encodeURIComponent(continuation)}`;const d=await eb(path);for(const t of d.transactions||[]){const amount=Number(t.transaction_amount?.amount||0)*(String(t.credit_debit_indicator||"").toUpperCase()==="DBIT"?-1:1);const isDebit=String(t.credit_debit_indicator||"").toUpperCase()==="DBIT",cp=(isDebit?(t.creditor?.name||t.creditor_name):(t.debtor?.name||t.debtor_name))||(t.creditor?.name||t.debtor?.name||t.creditor_name||t.debtor_name||""),iban=(isDebit?(t.creditor_account?.iban||t.creditor?.iban):(t.debtor_account?.iban||t.debtor?.iban))||(t.creditor_account?.iban||t.debtor_account?.iban||""),remit=Array.isArray(t.remittance_information)?t.remittance_information.join(" · "):t.remittance_information_unstructured||t.remittance_information||"",ext=t.entry_reference||t.transaction_id||`${a.external_account_id}-${t.booking_date||t.booking_date_time}-${amount}-${remit}`;await supa("bank_transactions?on_conflict=external_transaction_id",{method:"POST",prefer:"resolution=merge-duplicates,return=minimal",body:JSON.stringify({workspace_id:workspaceId,bank_account_id:a.id,external_transaction_id:String(ext),bank_name:a.bank_name,booking_date:String(t.booking_date||t.booking_date_time||new Date().toISOString()).slice(0,10),value_date:String(t.value_date||t.value_date_time||"").slice(0,10)||null,amount,currency:t.transaction_amount?.currency||a.currency||"EUR",counterparty_name:cp,counterparty_iban:iban,remittance_information:remit,raw:t})});imported++}continuation=d.continuation_key||null;loops++}while(continuation&&loops<10)}
 const fresh=await supa(`bank_transactions?workspace_id=eq.${workspaceId}&bank_account_id=in.(${(accts||[]).map(x=>x.id).join(',')||0})&match_status=eq.unmatched&amount=gt.0&select=*`);for(const tx of fresh||[])try{if(await autoPost(tx,workspaceId))autoMatched++}catch(e){console.error('auto-match',tx.id,e.message)}
 await supa(`bank_connections?id=eq.${id}&workspace_id=eq.${workspaceId}`,{method:"PATCH",body:JSON.stringify({last_sync_at:new Date().toISOString(),status:"connected",error_message:null})});res.status(200).json({ok:true,imported,autoMatched})}catch(e){res.status(500).json({error:e.message})}};
