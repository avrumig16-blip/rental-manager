...
async function autoPost(tx,workspaceId){
 ...
 const keepForReview=async()=>{await supa(`bank_transactions?id=eq.${tx.id}&workspace_id=eq.${workspaceId}`,{method:'PATCH',body:JSON.stringify({match_status:'suggested'})});return {posted:false,suggested:true};};
 // Pending/held bank items are useful suggestions but are never posted until booked.
 if(txStatus(tx.raw)!=='BOOK')return keepForReview();
 const settings=await workspaceBankMatchSettings(workspaceId);if(settings.mode==='review_only')return keepForReview();
 if(!b.reasons.includes('amount'))return keepForReview();
 if(settings.mode==='strict'){if(!['reference','iban','name','amount'].every(r=>b.reasons.includes(r)))return keepForReview();}
 else if(!b.reasons.includes('reference'))return keepForReview();
...
module.exports=async(req,res)=>{try{
 ...
 for(const a of accts||[]){if(!a.external_account_id)continue;let stale=false,apiBalance=null;
  try{const b=await eb(`/accounts/${encodeURIComponent(a.external_account_id)}/balances`);const pref={ITAV:1,CLAV:2,ITBD:3,CLBD:4,OPBD:5};const parsed=[...(b.balances||[])].map(bal=>{const box=bal.balance_amount||bal.balanceAmount||bal.amount||{},raw=typeof box==='object'?(box.amount??box.value):box,amount=Number(raw),type=String(bal.balance_type||bal.balanceType||'').toUpperCase(),at=bal.last_change_date_time||bal.lastChangeDateTime||bal.reference_date||bal.referenceDate||null,ts=Date.parse(at||'')||0;return{bal,box,amount,type,at,ts}}).filter(x=>Number.isFinite(x.amount));parsed.sort((x,y)=>(y.ts-x.ts)||((pref[x.type]||99)-(pref[y.type]||99)));let pick=parsed[0];if(pick){const sameFresh=parsed.filter(x=>x.ts===pick.ts),available=sameFresh.find(x=>['ITAV','CLAV'].includes(x.type)),booked=sameFresh.find(x=>['ITBD','CLBD'].includes(x.type));if(available&&!(available.amount===0&&booked&&Math.abs(booked.amount)>0))pick=available;else if(booked&&pick.amount===0&&Math.abs(booked.amount)>0)pick=booked;const currency=(typeof pick.box==='object'?(pick.box.currency||pick.box.currency_code):null)||pick.bal.currency||a.currency||"EUR";apiBalance={amount:pick.amount,currency,at:pick.at,type:pick.type};await supa(`bank_accounts?id=eq.${a.id}&workspace_id=eq.${workspaceId}`,{method:"PATCH",body:JSON.stringify({balance:apiBalance.amount,currency:apiBalance.currency,balance_updated_at:new Date().toISOString()})})}}
  ...
  try{do{
   ...
   const d=await eb(path);for(const t of d.transactions||[]){checked++;const status=txStatus(t);if(status==='CNCL'||status==='RJCT')continue;const isDebit=String(t.credit_debit_indicator||"").toUpperCase()==="DBIT",amount=Number(t.transaction_amount?.amount||0)*(isDebit?-1:1),cp=(isDebit?(t.creditor?.name||t.creditor_name):(t.debtor?.name||t.debtor_name))||(t.creditor?.name||t.debtor?.name||t.creditor_name||t.debtor_name||""),iban=(isDebit?(accountIban(t.creditor_account)||accountIban(t.creditor)):(accountIban(t.debtor_account)||accountIban(t.debtor)))||(accountIban(t.creditor_account)||accountIban(t.debtor_account)||""),remit=txReference(t),ext=t.entry_reference||t.transaction_id||`${a.external_account_id}-${t.booking_date||t.booking_date_time||t.transaction_date}-${amount}-${remit}`;
    const isNew=!existing.has(String(ext));await supa("bank_transactions?on_conflict=external_transaction_id",{method:"POST",prefer:"resolution=merge-duplicates,return=minimal",body:JSON.stringify({workspace_id:workspaceId,bank_account_id:a.id,external_transaction_id:String(ext),bank_name:a.bank_name,booking_date:String(t.booking_date||t.booking_date_time||t.transaction_date||new Date().toISOString()).slice(0,10),value_date:String(t.value_date||t.value_date_time||"").slice(0,10)||null,amount,currency:t.transaction_amount?.currency||a.currency||"EUR",counterparty_name:cp,counterparty_iban:iban,remittance_information:remit,match_status:"unmatched",raw:t})});if(isNew){existing.add(String(ext));newImported++;if(status==='PDNG'||status==='HOLD')pendingImported++;}
  ...
  if(latestTxBalance){const apiDay=String(apiBalance?.at||'').slice(0,10),txDay=String(latestTxBalance.when||'').slice(0,10),txIsFresher=!apiBalance||!apiDay||txDay>=apiDay;const fixesZero=apiBalance&&Number(apiBalance.amount)===0&&Math.abs(Number(latestTxBalance.amount||0))>0;if(txIsFresher||fixesZero){await supa(`bank_accounts?id=eq.${a.id}&workspace_id=eq.${workspaceId}`,{method:'PATCH',body:JSON.stringify({balance:latestTxBalance.amount,currency:latestTxBalance.currency,balance_updated_at:new Date().toISOString()})});apiBalance={amount:latestTxBalance.amount,currency:latestTxBalance.currency,at:latestTxBalance.when,type:'TX'}}}
 }
 const activeIds=(await supa(`bank_accounts?workspace_id=eq.${workspaceId}&connection_id=eq.${id}&active=eq.true&select=id`))||[];
 const fresh=await supa(`bank_transactions?workspace_id=eq.${workspaceId}&bank_account_id=in.(${activeIds.map(x=>x.id).join(',')||0})&or=(match_status.is.null,match_status.eq.unmatched,match_status.eq.suggested)&amount=gt.0&select=*`);for(const tx of fresh||[])try{const r=await autoPost(tx,workspaceId);if(r.posted)autoMatched++;else if(r.suggested)suggested++;}catch(e){console.error('auto-match',tx.id,e.message)}
