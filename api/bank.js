...
module.exports=async(req,res)=>{try{
 ...
 for(const a of accts||[]){if(!a.external_account_id)continue;let stale=false,apiBalance=null;
  ...
  try{do{
   let path=`/accounts/${encodeURIComponent(a.external_account_id)}/transactions?date_from=${new Date(Date.now()-120*86400000).toISOString().slice(0,10)}&strategy=longest`;if(continuation)path+=`&continuation_key=${encodeURIComponent(continuation)}`;
 ...
 const fresh=await supa(`bank_transactions?workspace_id=eq.${workspaceId}&bank_account_id=in.(${activeIds.map(x=>x.id).join(',')||0})&or=(match_status.is.null,match_status.eq.unmatched)&amount=gt.0&select=*`);for(const tx of fresh||[])try{const r=await autoPost(tx,workspaceId);if(r.posted)autoMatched++;else if(r.suggested)suggested++;}catch(e){console.error('auto-match',tx.id,e.message)}
