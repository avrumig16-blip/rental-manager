...
async function suggestionFor(tx,workspaceId){
 ...
 const text=norm([tx.counterparty_name,tx.counterparty_iban,tx.remittance_information].join(' ')), iban=clean(tx.counterparty_iban);let matches=[];
 ...
 matches.sort((a,b)=>b.score-a.score);const b=matches[0],second=matches[1];if(!b)return null;if(second&&b.score-second.score<5&&!b.reasons.includes('reference')&&!b.reasons.includes('iban'))return null;if(!b.reasons.some(r=>r!=='amount'))return null;return b;
}
...
module.exports=async(req,res)=>{try{
 ...
 for(const a of accts||[]){if(!a.external_account_id)continue;let stale=false,apiBalance=null;
  try{const b=await eb(`/accounts/${encodeURIComponent(a.external_account_id)}/balances`);const bal=(b.balances||[]).find(x=>["CLBD","CLAV","ITAV"].includes(String(x.balance_type||'').toUpperCase()))||(b.balances||[])[0];if(bal){apiBalance={amount:Number(bal.balance_amount?.amount||0),currency:bal.balance_amount?.currency||a.currency||"EUR",at:bal.last_change_date_time||bal.reference_date||null};await supa(`bank_accounts?id=eq.${a.id}&workspace_id=eq.${workspaceId}`,{method:"PATCH",body:JSON.stringify({balance:apiBalance.amount,currency:apiBalance.currency,balance_updated_at:new Date().toISOString()})})}}
  ...
  if(latestTxBalance&&(!apiBalance?.at||String(latestTxBalance.when)>=String(apiBalance.at).slice(0,10))){await supa(`bank_accounts?id=eq.${a.id}&workspace_id=eq.${workspaceId}`,{method:'PATCH',body:JSON.stringify({balance:latestTxBalance.amount,currency:latestTxBalance.currency,balance_updated_at:new Date().toISOString()})})}
 }
